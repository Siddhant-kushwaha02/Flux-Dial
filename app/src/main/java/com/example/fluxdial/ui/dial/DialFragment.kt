...
class DialFragment : Fragment() {
    ...
    private fun observeViewModel() {
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.callInitiated.collectLatest { callId ->
                if (callId != null) {
                    // Navigate to OutgoingCallActivity (to be built)
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.isLoading.collectLatest { isLoading ->
                binding.pbDialing.visibility = if (isLoading) View.VISIBLE else View.GONE
